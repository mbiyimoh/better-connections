...
**CRITICAL for drill series and curriculum generation:**

The system supports mathematical ground truth validation using external engines (like GNU Backgammon) to verify generated content.

**Key Components:**
- `lib/groundTruth/index.ts` - Barrel exports for all ground truth functionality
- `lib/groundTruth/types.ts` - Type definitions (GroundTruthConfig, VerificationClaim, etc.)
- `lib/groundTruth/tools.ts` - OpenAI function calling tool definitions
- `lib/groundTruth/executor.ts` - Tool execution against ground truth engine
- `lib/groundTruth/cache.ts` - Response caching with TTL
- `lib/groundTruth/config.ts` - Project configuration resolution
- `lib/groundTruth/promptBuilder.ts` - Prompt injection for verification
- `lib/groundTruth/verification/` - Claim extraction and verification

**How it works:**

1. **Configuration**: Projects can enable ground truth validation via `ProjectAssessment.useForContentValidation`
2. **Generation**: During drill/curriculum generation, GPT-4o is given tools to query the engine
3. **Tool Calling**: The model queries positions, verifies moves, gets equity values
4. **Post-Verification**: After generation, claims are extracted and verified against the engine
5. **Status Tracking**: `GuruArtifact.verificationStatus` shows VERIFIED, NEEDS_REVIEW, UNVERIFIED, or FAILED

**Verification Status Flow:**
```
Generation → Extract Claims → Verify Each → Calculate Status → Update Artifact
                                                    ↓
                              VERIFIED (all pass) / NEEDS_REVIEW (>30% fail) / FAILED (error)
```
...
if (gtConfig?.enabled) {
  ...
  const verification = await verifyClaimsAgainstGroundTruth(claims, gtConfig)
...
**UI Components:**
- `VerificationBadge` - Shows status with color-coded badge
- `VerificationDetailsModal` - Shows detailed verification results

**Caching:**
- Opening positions: 7 days TTL
- Specific positions: 24 hours TTL
- Move verifications: 1 hour TTL

**Limits:**
- MAX_TOOL_CALLS: 100 per generation
- MAX_ITERATIONS: 50 per generation
- ENGINE_QUERY_TIMEOUT: 10 seconds
- MAX_REGENERATION_ATTEMPTS: 5

**Gotchas:**
- Engine URL must be configured in AssessmentDefinition
- `canValidateContent` must be true on the AssessmentDefinition
- `useForContentValidation` must be true on the ProjectAssessment
- Verification failures don't block artifact completion (graceful degradation)

**Location:** `lib/groundTruth/index.ts`, `lib/groundTruth/verification/`
...
| Ground truth validation fails | Check AssessmentDefinition has `canValidateContent: true` and ProjectAssessment has `useForContentValidation: true` |
| Ground truth engine timeout | Check engine URL is accessible, increase ENGINE_QUERY_TIMEOUT if needed |
| Verification shows NEEDS_REVIEW | >30% of claims failed - review verificationDetails JSON for specific failures |
| Tool call limit exceeded | Reduce complexity of generated content or increase MAX_TOOL_CALLS limit |
